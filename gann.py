...
def validate_network_parameters(num_neurons_input,
    ...
    if not (num_solutions is None):
        if num_solutions < 2:
            raise ValueError(
                "num_solutions: The number of solutions within the population must be at least 2. The current value is {num_solutions}.".format(
    ...
    if not (type(num_neurons_hidden_layers) in [list, tuple]):
        raise TypeError(
            "num_neurons_hidden_layers: A list or a tuple is expected but {hidden_layers_neurons_type} found.".format(
                hidden_layers_neurons_type=type(num_neurons_hidden_layers)))
    ...
    if not (output_activation in supported_activations):  # activation_type
        raise ValueError(unexpected_activation_value.format(activations=output_activation,
    ...
    if num_hidden_layers > 1:  # In case there are more than 1 hidden layer.
        if type(hidden_activations) in [list, tuple]:
            ...
            if num_activations != num_hidden_layers:
                raise ValueError(length_mismatch.format(
                    mismatched_lengths="{num_activations} != {num_layers}".format(num_layers=num_hidden_layers,
            ...
            if hidden_activations in supported_activations:
                ...
                raise ValueError(unexpected_activation_value.format(supported_activations=supported_activations,
        ...
        print(
            "WARNING: There are no hidden layers however a value is assigned to the parameter 'hidden_activations'. It will be reset to [].".format(
                hidden_activations=hidden_activations))
        hidden_activations = []
    ...
    for act in hidden_activations:
        if not (act in supported_activations):
            raise ValueError(
                unexpected_activation_value.format(supported_activations=supported_activations, activations=act))
