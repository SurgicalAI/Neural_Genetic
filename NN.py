...
def train(num_epochs,
    ...
    if not (problem_type in ["classification", "regression"]):
        raise ValueError(
            "The value of the problem_type parameter can be either classification or regression but {problem_type_val} found.".format(
...
def predict(last_layer, data_inputs, problem_type="classification"):
    ...
    if len(weights) != len(activations):
        raise TypeError(
            "The length of layers {num_layers} is not equal to the number of activations functions {num_activations} and they must be equal.".format(
                num_layers=len(weights), num_activations=len(activations)))
...
def to_vector(array):
    ...
    if not (type(array) is numpy.ndarray):
        raise TypeError("An input of type numpy.ndarray is expected but an input of type {in_type} found.".format(
            in_type=type(array)))
...
def to_array(vector, shape):
    ...
    if vector.ndim > 1:
        raise ValueError(
            "A 1D NumPy array is expected but an array of {ndim} dimensions found.".format(ndim=vector.ndim))
    if vector.size != functools.reduce(lambda x, y: x * y, shape, 1):  # (operator.mul == lambda x,y:x*y
        raise ValueError(
            "Mismatch between the vector length and the array shape. A vector of length {vector_length} cannot be converted into a array of shape ({array_shape}).".format(
                vector_length=vector.size, array_shape=shape))
...
class InputLayer:
    ...
    def __init__(self, num_inputs):
        if num_inputs <= 0:
            raise ValueError(
                "Number of input neurons cannot be <= 0. Please pass a valid value to the 'num_inputs' parameter.")
        ...
        if not (activation_function in supported_activation_functions):
            raise ValueError(
                "The specified activation function '{activation_function}' is not among the supported activation functions {supported_activation_functions}. Please use one of the supported functions.".format(
                    ...
                    supported_activation_functions=supported_activation_functions))
